...
async function seedDatabase() {
  try {
    console.log('🌱 Inizio seeding database MySQL...\n');

    // Controlla se esistono già oggetti
    const existingObjects = await db.query('SELECT COUNT(*) as count FROM astronomical_objects');
    ...
    if (parseInt(existingObjects.rows[0].count) > 0) {
      ...
      await db.query('TRUNCATE TABLE astronomical_objects');
